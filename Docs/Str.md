...
The `startsWith()` method is used to determine whether the given string starts with the specified substring. It compares the beginning of the string with the provided substring and returns `true` if they match, otherwise `false`.
...
The `matchesRegex ()` method allows you to check if a given string matches a specified regular expression pattern. It utilizes PHP's `preg_match` function to perform the matching operation. The method returns `true` if the regular expression pattern is found within the string and `false` otherwise.
...
#### trimLeft($characters = null)

The `trimLeft()` method enables easy string manipulation in a PHP application. This method is designed to remove leading whitespace or specific custom characters from the start of the string.
...
#### trimRight($characters = null)

The `trimLeft()` method enables easy string manipulation in a PHP application. This method is designed to remove trailing whitespace from the end of a given string. It offers the flexibility to specify additional characters to remove from the right end of the string, in addition to the standard whitespace characters.
...
The `trimLeft()` method designed to remove a specific substring from the original string. It employs the `str_replace()` function to replace occurrences of the provided `$substring` with an empty string (''). This effectively removes all instances of the given substring from the original string.
...
The `removeAll()` method to remove all occurrences of a specific character from the provided string.
...
The `removePrefix()` method to remove a specific prefix from the given string, if it exists. It is useful for scenarios where you need to manipulate strings by removing a known prefix, such as removing a common identifier from the beginning of a string.
...
The `removeSuffix()` method to remove a specified suffix from the end of the string if it exists. It takes one parameter, `$substring`, which represents the suffix to be removed.
...
The `replaceMultiple()` method allows to replace multiple occurrences of substrings in the string with corresponding replacement strings. This method takes an associative array of `$replacements` as its parameter, where the keys represent the substrings to be replaced, and the values represent the strings to replace them with.
**Parameters:**
...
#### removeMultiple($substrings)

The `removeMultiple()` method allows to remove all occurrences of multiple substrings from the string. It takes an array of substrings as its parameter and returns the resulting string with the specified substrings removed.
